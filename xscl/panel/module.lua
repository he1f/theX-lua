...
local function resolve_runtime_column_types(handle, whatpanel, pinfo)
  local types = call_panel_method(panel.GetColumnTypes, handle, whatpanel)
  if type(types) == "string" and types ~= "" then
    return types
  end
  if type(pinfo) == "table" and type(pinfo.ColumnTypes) == "string" and pinfo.ColumnTypes ~= "" then
    return pinfo.ColumnTypes
  end

  local fallback = call_panel_method(panel.GetPanelFormat, handle, whatpanel)
  if type(fallback) == "string" and fallback:match("^%s*[A-Z0-9,]+%s*$") and fallback:find(",") then
    return fallback
  end
  return nil
end

local function resolve_mode_config(view_mode, runtime_types)
  ensure_panel_modes()
  if type(view_mode) == "number" then
    ...
    for i = 1, #mode_candidates do
      local mode_index = mode_candidates[i]
      if type(mode_index) == "number" then
        local mode = panel_modes[mode_index]
  ...
  if type(runtime_types) == "string" and runtime_types ~= "" then
    for i = 1, #panel_modes do
      if panel_modes[i].ColumnTypes == runtime_types then
        return panel_modes[i]
      end
    end
  end
  return nil
...
local function find_c0_index(types_line)
  local types = split_csv(types_line or "")
  for i = 1, #types do
...
local function normalize_widths(widths_value)
  if type(widths_value) == "string" then
    return split_csv(widths_value)
  end
  if type(widths_value) == "table" then
    local out = {}
    for i = 1, #widths_value do
      out[i] = tostring(widths_value[i] or "")
    end
    return out
  end
  return {}
...
local function looks_like_xscl_c0_widths(widths)
  if type(widths) ~= "table" then
    return false
  end
  if #widths == 4 then
    local w2 = tonumber(widths[2]) or 0
    local w3 = tonumber(widths[3]) or 0
    local w4 = tonumber(widths[4]) or 0
    return w2 == 5 and w3 == 5 and w4 == 3
  end
  if #widths == 2 then
    local w1 = tonumber(widths[1]) or 0
    local w2 = tonumber(widths[2]) or 0
    return w1 == 12 or w2 == 12
  end
  return false
end
